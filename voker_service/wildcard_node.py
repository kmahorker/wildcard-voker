...
async def run_single_voker(voker, voker_system_prompt, auth_config, previous_messages, voker_inputs = None):
    tool_client, openai_client = await init_tool_node(voker["tool_name"], auth_config, "")
    
    # Process Voker Inputs
    if voker_inputs is not None:
        for key, value in voker_inputs.items():
            voker_system_prompt = voker_system_prompt.replace(f"@{key}", value)
